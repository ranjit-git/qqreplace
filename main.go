...
func main() {
	var once bool
	flag.BoolVar(&once, "once", false, "replace all param at once")
	...
	for sc.Scan() {
        ...
        uu :=u
          //fmt.Printf("hi")
        query_part :=u.Query()
		for param, vv1 := range query_part {
			if appendMode {
				qs.Set(param, vv1[0]+flag.Arg(0))
			} else { 		
		...
		for param2, vv2 := range query_part {
             if param2 !=param {
                  qs.Set(param2, vv2[0])
                    }
             } 
